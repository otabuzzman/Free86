...
uint32_t Free86::compile_SFLAGS() {
    uint32_t  f0 = is_CF() << 0;
    uint32_t f2 = is_PF() << 2;
    uint32_t f4 = is_AF() << 4;
    uint32_t f6 = (osm_dst == 0) << 6;
    uint32_t f7 = (osm == 24 ? ((osm_src >> 7) & 1) : (osm_dst >> 31) & 1) << 7;
    uint32_t f11 = is_OF() << 11;
    return f11 | f7 | f6 | f4 | f2 | f0;
}
uint32_t Free86::compile_EFLAGS() {
    uint32_t bits = compile_SFLAGS();
    bits |= df & 0x00000400;
    bits |= eflags;
    return bits;
}
void Free86::update_EFLAGS(uint32_t bits, uint32_t mask) {
